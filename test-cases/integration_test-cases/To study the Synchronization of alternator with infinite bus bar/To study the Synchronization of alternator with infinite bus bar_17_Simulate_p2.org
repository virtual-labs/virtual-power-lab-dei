...
* Test Steps
  ...
  4. Don't enter the data in the input fields ( Change in Speed(rpm),Variation in Rf(Ω)).
  5. Click on the ' Calculate Generator Voltage and Frequency' button
...
* Expected result
  ...
  4. The result should not be displayed in the output fields (Frequency(Hz),Terminal Voltage(V)) and an error message should be displayed at the input fields as to enter the valid values.
